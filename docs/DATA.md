...
| **US Census Bureau** — ZCTA↔county relationship + CBSA delineation (public domain, US Government works) | `tab20_zcta520_county20_natl.txt` + `list1_2023.xlsx` → `pipeline/data/zip_cbsa.csv` | Committed; re-run `fetch_cbsa.py` per OMB delineation revision (~5-yearly) | ZIP→metro mapping for the Research index's metro league tables. Chain: ZIP≈ZCTA → county (largest land-area overlap) → CBSA. The ZCTA≈ZIP approximation is the standard documented compromise — USPS publishes no ZIP geography. 24,734 ZIP rows, 928 CBSAs, 2023 delineation vintage. |
