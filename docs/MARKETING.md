...
## 2026-08-11 — recap_thread

The monthly recap ships: one X thread, a lead plus four replies plus a closer,
generated by `cand_recap` in `pipeline/marketing_tasks.py`.

**A thread is ONE post that happens to be many rows.** Every rule in the queue
treats it that way — it is planned as one candidate, consumes one slot, spends
one post of the weekly cap, and counts once in the mix meter. It only becomes
six rows in `rows_from_placement`, the very last step before insert, because six
is what the operator has to paste. The predicate that expresses this everywhere
is `coalesce(thread_position, 0) = 0` — "this row is a post".

**Four things made threads impossible before v31**, and v27 added the columns
without changing any of them:

| # | rule | what it did to a thread |
|---|---|---|
| 1 | `marketing_tasks_slot_idx` (UNIQUE on channel, scheduled_for) | rejected reply 1 |
| 2 | conflict R4, "slot taken" | rejected reply 1 |
| 3 | conflict R2, weekly cap | counted 6 rows against a cap of 3 |
| 4 | conflict R3, metro cooldown | a roundup naming 5 metros locked all 5 out for a fortnight |

**v31 was itself wrong and v33 fixes it.** v31 decided "is this a reply?" by
looking the row up by id — but `marketing_tasks_caps` is a BEFORE INSERT
trigger, so the row is not in the table yet. The lookup found nothing, every
reply was judged a full post, and R4 refused reply 1 exactly as before. The
whole migration was inert. **Reading the SQL could not have shown this** — it is
correct in isolation and only wrong given when it runs. It was caught by
inserting a real six-row thread against production inside a rolled-back
transaction. The position is now passed in, which moved all three callers.

**Only the lead carries a link**, so replies get no `/go/` redirect and no
`utm_url` — five dead redirect pages and five permanently-zero campaign tokens
otherwise. `lint_caption(reply=True)` relaxes exactly two rules (link,
attribution) and keeps every other one.

### Bugs this found in already-queued posts

**Streak claims crossed the source seam.** `streaks.json` advances across the
whole archive including the reconstructed tracker-v1 months, so it holds runs
of 89 months against a continuous series only 73 long — an 89-month run ending
June 2026 begins February 2019, sixteen months the wrong side of the seam.
`research.py`'s own contract says streak-facing claims never reach across it,
and PR3 shipped three spotlight posts (11354, 33139, 15222) claiming 89, 86 and
74 months. Now clamped to the continuous basis and phrased as "in all 73 months
we have measured", which is the stronger claim anyway.

**"U.S." always failed the all-caps lint.** `\b` terminates before the trailing
period, so the match was `U.S` and `"U.S".strip(".")` never equalled the
permitted `US`. Comparison now strips every period. Existing templates dodged it
by never writing "U.S." in a caption.

### What the thread says, and where each number comes from

Nothing is recomputed; `run_length` in particular is read from
`research.detect_records()`, which is the fix for the card that once published
"fifth month in a row" against a truth of three.

| row | claim | source |
|---|---|---|
| 0 lead | index fell for the Nth month, to X% | `records.wsi`, `run_length`, `run_direction` |
| 0 | down from the peak | `max()` of `national_series(segment="continuous")` |
| 1 | "falling is not the same as low" — the floor | `min()` of the same series |
| 2 | fell in N of 51 state rollups | `state_moves` deltas |
| 3 | N ZIPs crossed into warning | `len(flips_to_warning)` |
| 4 | one metro, same denominator both months | `history.json` metro counts |
| 5 | ~25,000 ZIPs, 73-month record, free files | `national.scored`, `basis_months` |

Reply 4 exists because the national scored set grew by 685 ZIPs this month, so
almost every cross-month COUNT comparison is confounded by which places were
measurable. A metro with an identical denominator in both months is the one
place a reader can be handed two raw counts and trust the subtraction.

**Not used, deliberately:** `diff_verdicts()` buckets (they count arrivals at a
