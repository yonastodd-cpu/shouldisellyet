...
| **Final close-out: review rows 2, 4/19, 19a/19c, 22 fixed and re-verified** | 2026-08-28T22:45:00Z | Row 2: archived research releases (2026-06, 2026-07) now render the methodology changelog's unnamed form ("computed from a prior data vendor's sold-home statistics"); fixed in the `build_research.py` template (the pages are generated each deploy) and the gate recursive-swept — the vendor name is permitted nowhere in rendered output except the methodology changelog, zero other exceptions (`test_basis_disclosure.py`). Rows 4/19: `deploy` now **needs** `verify-released` in `update.yml`; verified live by a deliberate forbidden-string branch dispatch (run 33215236015: refresh ✓, verify-released ✗, **deploy skipped**), branch deleted. Rows 19a/19c: stale-data alert (`pipeline/alert_stale_data.py`, scheduled refresh, 45-day line) and Stripe-webhook error-path alert (`alertOps`, deployed) built; both `[TEST]` sends delivered from CI (run 33216902552) after fixing a fleet-wide Cloudflare bot-block on urllib's default User-Agent (403/1010) that silently covered EVERY pipeline Resend sender — **operator inbox confirmation outstanding**. Row 22: OG tags on `/methodology.html` + `/pricing/`; `audit-og.py` now asserts basis-correctness (prior vendor, v1 vocabulary, stale dates, missing tags) instead of figure-absence — 93 priority URLs clean. Gates A–E re-run against production post-deploy: **A clean · B pass · C pass · D 725 passed · E 93/93 clean.** |
| **Paid-report dead end: found, root-caused, fixed, deployed (same day)** | 2026-08-28T22:45:00Z | A real customer (report, ZIP 20005, purchased 21:51:34Z) was refused by `my-report.html` with waitlist copy although their checkout pull had stored the data at 21:50:56Z — root cause: `market-reading` cached its misses for a day, and the browser that cached the 21:00 miss was the browser that needed the 21:51 reading. Fixes, all live: misses now cache 300 s; the report page fires the SAME on-demand pull at report-build time on every paid path (cache-busted re-fetch first, then `{zip, token}` — token verified server-side, paid pulls run even at the monthly ceiling, same floor/ledger/dedupe, schema-v42 `paid_coverage_gap` outcome + operator email on paid floor-failure); a paid failure builds the partial report (equity + national context, honest copy, refund on reply) — never waitlist copy, now forbidden on paid surfaces by gate B and `test_report_paid_path.py`. Privacy copy corrected to what the page actually sends (build path: ZIP only — asserted in test). 20005 passed the floor (ACT, 3 signals), `/zip/20005/` promoted and live; the customer's token verified against the deployed paid path (`ok:true, served:"store"`). |
| **Rows 19a/19c CLOSED — both alert emails confirmed in the operator's inbox** | 2026-08-28T22:52:05Z | Operator confirms receipt of both `[TEST]` sends from run 33216902552: "[TEST] Stale-data alert — delivery check" (the exact script the scheduled refresh runs, `pipeline/alert_stale_data.py --test`) and "[TEST] Stripe webhook needs attention — delivery check" (same key, sender and recipients as the deployed function's `alertOps`). With row 19b closed 2026-08-27, all three legs of `FINAL_REVIEW.md` row 19 are now **live-tested to the inbox, not assumed**: gate failures block the deploy outright (sequencing fix), stale data emails from the Mon/Thu refresh, and a webhook error-path failure emails per occurrence. |
